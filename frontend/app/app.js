...
function renderDashboard() {
  ...
  setText("strongestDirection", topCandidate?.candidate_type || dashboard.strongest_candidate_type || "-");
  setText("marketState", marketState);
  setText("screeningStatus", marketState === "defense" ? "DEFENSIVE FILTER ONLY" : "YES, WITH TRIGGERS");
...
function renderCandidateTable() {
  ...
  tbody.innerHTML = (dashboard.top_candidates || []).map(candidate => `
    <tr data-ticker="${safeAttr(candidate.ticker)}">
      ...
      <td><strong>${safe(candidate.ticker)}</strong><br><span class="badge ${edgeClass(candidate.edge_status)}">${safe(candidate.edge_status || "-")}</span></td>
      <td>${safe(candidate.candidate_type)}</td>
      <td>${safe(candidate.elasticity_score)}</td>
      <td>${safe(candidate.confluence_score)}</td>
      <td>${safe(candidate.catalyst_score)}</td>
      <td>${safe(candidate.risk_score)}</td>
      <td>${safe(candidate.primary_scenario)}</td>
...
function renderDetail() {
  ...
  setText("detailTitle", `${candidate.ticker} · ${candidate.company_name || "Unknown company"}`);
  ...
  document.getElementById("candidateDetail").innerHTML = `
    ...
    <div class="detail-card">
      ...
      ${facts([
        ...
        ["次日区间", rangeLabel(candidate.next_day_expected_range)],
      ...
      <p class="note">这些是概率路径点位，不是投资建议或交易指令。</p>
    </div>
    <div class="detail-card">
      <h3>催化 / 相对强弱</h3>
      ${facts([
...
function renderValidation() {
  ...
  document.getElementById("validationGrid").innerHTML = [
    ...
    ["Trigger-condition return", pct(baseline.avg_trigger_condition_return)],
...
function analogSummary(analog) {
  if (!analog) return "<p>No analog summary.</p>";
  return facts([
...
function evidence(items, conflict) {
  ...
  return `<div class="evidence">${items.map(item => `
    <div class="evidence-item ${conflict ? "conflict" : ""}">
      <strong>${safe(item.name || item.source || "evidence")} · ${safe(item.score ?? "-")}</strong>
